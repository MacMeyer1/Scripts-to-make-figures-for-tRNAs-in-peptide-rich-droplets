...
for (var in files)
{
  ...
  {
    print(pos)
    if (Reac[pos, "Modified_effective_depth"] < 5000) {
      Reactivity <- 0 
    } else {
...
#### Make the heatmap ####
NM_10mers_12_10_heatmap <- ggplot(Alignment_melt_sort2, aes(x = Position, y = tRNA, fill = countfactor))+
...
NM_10mers_12_10_heatmap

#### Save your heatmap as a .svg ####
ggsave(filename = 'NM_10mers_12_10_heatmap.svg', path = getwd(), plot = NM_10mers_12_10_heatmap, scale = 10, width = 5, height = 5, units = "cm", dpi = 300)