...
for (var in files)
{
  ...
  tRNA_name <- strsplit(var, "merge_")[[1]][2]
  ...
  {
    print(pos)
    if ((Reac[pos, "Modified_effective_depth"] < 5000)|(Reac[pos, "Untreated_effective_depth"] < 5000)) {
      Reactivity <- 0 
    } else {
...
Alignment_melt_sort2 <- transform(Alignment_melt_sort2, countfactor = factor(countfactor, levels = c("no data", ">10", "10~3", "3~2", "2~1", "1~0.6", "0.6~0.3", "<0.3")))
...
my_pal <- c("#FFF7EC", "#FEE8C8", "#FDD49E", "#FDBB84", "#FC8D59", "#EF6548", "#D7301F", "grey90", "white")

#### Make the heatmap ####
T7_10mers_11_05_heatmap <- ggplot(Alignment_melt_sort2, aes(x = Position, y = tRNA, fill = countfactor))+
  ...
  scale_fill_manual(aesthetics = "fill", values=rev(my_pal), na.value="white", name = "Reactivity", limits = c("no nts", "no data", ">10", "10~3", "3~2", "2~1", "1~0.6", "0.6~0.3", "<0.3"))+
  theme(axis.text.x = element_text(angle = 90, hjust = 0))+
  theme(legend.position="right", legend.direction="vertical")
T7_10mers_11_05_heatmap

#### Save your heatmap as a .svg ####
ggsave(filename = 'T7_10mers_11_05_heatmap.svg', path = getwd(), plot = T7_10mers_11_05_heatmap, scale = 10, width = 5, height = 5, units = "cm", dpi = 300)